...
Matrix NewMatrix(size_t num_rows, size_t num_cols){
    ...
    // Allocate memory for 2d array of type float within matrix struct
    newMatrix->index = (float**)calloc(newMatrix->num_rows, sizeof(float*));
    for(int i = 0; i < newMatrix->num_rows; i++)
        newMatrix->index[i] = (float*)calloc(newMatrix->num_cols, sizeof(float));
    return newMatrix;
}
...
float Determinant(Matrix matrix){
    ...
    float determinant_multiplier = ReducedRowEchelonForm(matrix);
    int determinant = 1;
...
Matrix Cholesky(Matrix matrix){
    ...
    // loop over each row
    for(int i = 0; i < matrix->num_rows; i++){
        float temp_for_diag = 0;
        // calculate non-diagonal value
        for(int j = 0; j < i; j++){
            float temp_for_non_diag = 0.0;
            for(int k = 0; k < j; k++)
                // calculate non-diagonal value
...
void SwapRows(Matrix matrix, int rowA, int rowB){
    for(int i = 0; i < matrix->num_cols; i++){
        float temp = matrix->index[rowA][i];
        matrix->index[rowA][i] = matrix->index[rowB][i];
        matrix->index[rowB][i] = temp;
    }
}

void DivideRow(Matrix matrix, int row, float divisor){
    for(int i = 0; i < matrix->num_cols; i++)
        matrix->index[row][i] /= divisor; // Reduce row by dividing by a common divisor
}


void AddMultipleRow(Matrix matrix, int row_receiver, int row_multiple, float scalar){
...
void SwapValues(float *index_one, float *index_two){
    float temp_value = *index_one;
    *index_one        = *index_two;
