...
typedef struct{
    float **index;  // pointer to 2D array of matrix values
    size_t num_rows;
    size_t num_cols;
}matrix_struct;
...
/*************************************************************************
 * float Reduced_row_echelon_form(Matrix matrix)
 *
 ...
 *             Determinant(Matrix matrix, float determinant_multiplier)
 ...
 *        See: float Reduced_row_echelon_form(..)
 *
 * -> PARAMETERS:
 *    matrix                 - a matrix structure
 *
